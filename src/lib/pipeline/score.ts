...
import type { Crime } from "@/lib/slop";
import type { CrawlResult } from "./github";
import type { Receipt } from "@/lib/slop";

/**
 * Analysis agent — the Slop Score, computed deterministically from dates,
 * counts, and ratios (spec §4: scores must be consistent and defensible when
 * people argue about them). Every sub-score also emits citable crimes.
 */
...
const LAZY_COMMIT =
  /^(fix|fixes|fixed|wip|update|updates|updated|asdf+|test|testing|stuff|minor|misc|changes|final|final final.*|\.+|cleanup|clean up|tweak|tweaks|oops|typo|temp|aaa+|qwe.*|todo)$/i;
...
export function computeScore(
  ...
  const daysSince = lastDate ? Math.floor((Date.now() - lastDate) / DAY) : 999;

  if (daysSince >= 365) abandonment = 96;
  else if (daysSince >= 180) abandonment = 82;
  else if (daysSince >= 90) abandonment = 64;
  else if (daysSince >= 30) abandonment = 38;
  else if (daysSince >= 7) abandonment = 18;
  ...
  if (commits.length > 0) {
    ...
    const singleWord =
      commits.filter((c) => !c.message.trim().includes(" ")).length /
      commits.length;
    ...
    const gem = commits.find((c) =>
      /final final|asdf|revert revert|^\.+$/i.test(c.message),
    );
    if (gem) {
      crimes.push({
        category: "commit_poetry",
  ...
  if (comingSoon) readmeCope += 18;
  readmeCope += deadDemoLinks.length * 22;
  if (readme.length < 200 && meta.stars < 5) readmeCope = Math.min(readmeCope, 25);
  ...
  if (hasNodeModules) vibeCheck += 40;
  if (hasEnvFile) vibeCheck += 32;
  ...
  const derivativeName =
    /clone|wrapper|gpt|ai-|-ai$|awesome-|todo|portfolio|url-short/i.test(
      meta.name,
    );
  if (derivativeName) originality += 12;
